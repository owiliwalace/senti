...
const PQ1: React.FC = () => {
    ...
    const [copyOfSpecialGroup, setCopyOfSpecialGroup] = useState<any>(null);
    ...
    useEffect(() => {
        const fetchUploadedDocuments = async () => {
            ...
            if (user) {
                ...
                const q = query(collection(db, 'PQ1'), where('email', '==', userEmail));
    ...
    const selectFile = async (setFile: React.Dispatch<React.SetStateAction<any>>, fieldName: string) => {
        try {
            ...
            if (!result.canceled && result.assets && result.assets.length > 0) {
                ...
                setFile(null); // Reset to null if the user cancels the selection
            }
        } catch (error) {
    ...
    const uploadFiles = async () => {
        ...
        try {
            ...
            await Promise.all([
                ...
                handleFileUpload(copyOfSpecialGroup, 'CopyOfSpecialGroup'),
            ]);
            ...
            <BottomSheetModalProvider>
                <View>
                    ...
                    <View style={styles.documentContainer}>
                        ...
                        <Text style={{ color: textColor }}>{`Copy of PIN: ${renderLatestDocument('CopyOfPIN')}`} </Text>
                        <Text style={{ color: textColor }}>{`Certificate of Registration: ${renderLatestDocument('CopyOfCertificateRegistration')}`} </Text>
                        <Text style={{ color: textColor }}>{`Tax Compliance: ${renderLatestDocument('CopyOfTaxCompliance')}`} </Text>
                        <Text style={{ color: textColor }}>{`CR12: ${renderLatestDocument('CR12')}`} </Text>
                        <Text style={{ color: textColor }}>{`Business Permit: ${renderLatestDocument('CopyOfBusinessPermit')}`} </Text>
                        <Text style={{ color: textColor }}>{`Special Group: ${renderLatestDocument('CopyOfSpecialGroup')}`} </Text>
                    </View>
                    <View>
                        <Text style={{ fontWeight: 'bold', color: textColor, textAlign: 'center', fontSize: 16 }}>Upload New Documents</Text>
                        {renderFileSelection(copyOfPIN, setCopyOfPIN, 'CopyOfPIN')}
                        {renderFileSelection(copyOfCertificateRegistration, setCopyOfCertificateRegistration, 'CopyOfCertificateRegistration')}
                        {renderFileSelection(copyOfTaxCompliance, setCopyOfTaxCompliance, 'CopyOfTaxCompliance')}
                        {renderFileSelection(cr12, setCR12, 'CR12')}
                        {renderFileSelection(copyOfBusinessPermit, setCopyOfBusinessPermit, 'CopyOfBusinessPermit')}
                        {renderFileSelection(copyOfSpecialGroup, setCopyOfSpecialGroup, 'CopyOfSpecialGroup')}
                    ...
                    <BottomSheetModal ref={bottomSheetModalRef} index={1} snapPoints={snapPoints}>
                        <View style={styles.contentContainer}>
                            <Text style={{ textAlign: 'center' }}>Hello! I am a Bottom Sheet Modal 😊</Text>
                        </View>
                    </BottomSheetModal>
                </View>
            </BottomSheetModalProvider>
